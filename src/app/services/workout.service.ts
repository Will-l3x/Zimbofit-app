...
import { map, tap, take, switchMap, reduce } from 'rxjs/operators';
...
export class WorkoutService {
  ...
  getWorkout(id): Observable<any> {
    const workout$: Observable<any> = this.afs
      .doc(`workouts/${id}`)
      .valueChanges();
    const authoredWorkout$: Observable<any> = this.afs
      .doc(`user-authored-workouts/${id}`)
      .valueChanges();
    const exercises$ = this.exerciseCollection.valueChanges();
    const purchases$ = this.purchasesService.getPurchases();
  ...
  async getWorkoutSessionExercises(workout): Promise<SessionExercise[]> {
    const allExercises = await this.exerciseCollection
      .valueChanges()
      .pipe(take(1))
      .toPromise();
  ...
  async setupMeasurements(exercise) {
    if (exercise) {
      ...
      exercise.sets.map((set) => {
        ...
        const exerciseTime = exercise.measurements.find(
          (measure) => measure.name === 'time'
        );
        const exerciseWeight = exercise.measurements.find(
          (measure) => measure.name === 'weight'
        );
        const exerciseReps = exercise.measurements.find(
          (measure) => measure.name === 'reps'
        );
        const exerciseDistance = exercise.measurements.find(
          (measure) => measure.name === 'distance'
        );
